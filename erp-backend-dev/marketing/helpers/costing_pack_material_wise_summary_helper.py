...
from marketing.models import OrderPackItem, OrderPackItemPlacement, OrderPackPlacement, OrderPackItemPlacementMaterial, \
    ...
    OrderPackOtherCost, SupplierInquiryDetail
...
class MaterialCostingSummaryMixin:
    # TODO - major should the dividing total cost be the total quantity of all the packs
    def __init__(self, costing, packs):
        self.costing = costing
        self.packs = packs.filter(version=costing)
        ...
        for material_row in material_data:
            ...
            for supplier_inquiry_detail_id, consumption_data in consumption_summary.items():
                ...
                for row in consumption_data['consumption_summary']:
                    ...
                    if supplier_inquiry_detail.supplier_inquiry.customer_brand_material.id == 452:
                        print(average_consumption, total_quantity, row_consumption, row_quantity)
    ...
    def get_fabric_material_summary(self, placement_materials, consumption_ration_field_name, get_quantity_func):
        material_data = {}
        for placement_material in placement_materials:
            material_category = placement_material.material.material_category
            material_category_display = placement_material.material.material_detail.generic_material.user_material.get_category_display()
            headers = UserDefinedMaterial.get_material_headers(placement_material.material.material_detail.generic_material.user_material.name)

            if placement_material.material_id not in material_data:
                material_data[placement_material.material_id] = {
                    'material_detail': placement_material.material.get_attributes(),
                    'headers': headers,
                    'consumption_summary': {},
                    'material_type': placement_material.material.material_type,
                    'material_label': placement_material.material.material_label,
                    'material_category': {'category': material_category, 'category_display': material_category_display},
                }

            if isinstance(placement_material, OrderPackPlacementMaterial):
                supplier_inquiry_detail = placement_material.placement.order_pack.get_selected_supplier_inquiry_for_material(placement_material.material).supplier_inquiry_detail
            else:
                supplier_inquiry_detail = placement_material.placement.order_pack_item.pack.get_selected_supplier_inquiry_for_material(placement_material.material).supplier_inquiry_detail

            consumption_ratio = placement_material.get_order_item_fabric_costing_consumption(supplier_inquiry_detail)

            if supplier_inquiry_detail.pk not in material_data[placement_material.material_id]['consumption_summary']:
                material_data[placement_material.material_id]['consumption_summary'][supplier_inquiry_detail.pk] = {
                    'consumption_summary': []
                }

            material_data[placement_material.material.id]['consumption_summary'][supplier_inquiry_detail.pk]['consumption_summary'].append({
    ...
    def get_material_summary(self, placement_materials, consumption_ration_field_name, get_quantity_func):
        material_data = {}
        for placement_material in placement_materials:
            material_category = placement_material.material.material_category
            material_category_display = placement_material.material.material_detail.generic_material.user_material.get_category_display()
            consumption_ratio = getattr(placement_material, consumption_ration_field_name)
            headers = UserDefinedMaterial.get_material_headers(placement_material.material.material_detail.generic_material.user_material.name)
    ...
    def get_ie_operation_costs_summary(self, pack_items):
        ...
        for pack_item in pack_items:
            item_operations = pack_item.get_colorway_item_operations()
            ...
            for item_operation in item_operations:
                ...
                pack_item_epm = pack_item.pack.version.get_version_earnings_per_minute(pack_item)
                data[item_operation_id]['average_epm'] = (float(data[item_operation_id]['costing_smv']) * float(data[item_operation_id]['average_epm']) *
                                                                 float(data[item_operation_id]['total_pack_item_quantity']) + float(pack_item_epm) * float(item_operation.costing_smv) * float(pack_item.pack.cad_quantity)) / (
                                                                 data[item_operation_id]['total_pack_item_quantity'] + pack_item.pack.cad_quantity
                                                         )
                data[item_operation_id]['average_total_cost'] = data[item_operation_id]['average_epm'] * data[item_operation_id]['costing_smv']

                data[item_operation_id]['total_pack_item_quantity'] += pack_item.pack.cad_quantity
                data[item_operation_id]['operation_summary'].append({'pack_item_display': pack_item.get_pack_item_display(), 'quantity': pack_item.pack.cad_quantity, 'epm': pack_item_epm})
        return data.values()
    ...
    def get_other_costs(self):
        ...
        for pack in self.packs:
            pack_other_costs = pack.get_other_costs()
            total_quantity = 0
            for pack_other_cost in pack_other_costs:
                ...
                total_quantity += pack.cad_quantity
                data[other_cost_type_id]['other_cost_summary'].append({'pack_display': pack.get_pack_display(), 'quantity': pack.cad_quantity})
    ...
    def calculate_top_level_summary(self):
        ...
        for pack in self.packs:
            cost_summary = pack.get_cost_summary()
            pack_quantity = pack.cad_quantity
...
class MaterialWiseSummary(MaterialCostingSummaryMixin):

    def get_cost_summary(self):
        get_quantity_func = lambda placement_material_object: placement_material_object.placement.order_pack_item.pack.cad_quantity
        get_quantity_func_b = lambda placement_material_object: placement_material_object.placement.order_pack.cad_quantity
        order_pack_items = OrderPackItem.objects.filter(pack__in=self.packs)
        pack_item_placements = OrderPackItemPlacementMaterial.objects.filter(placement__order_pack_item__pack__in=self.packs).exclude(
            material__material_detail__generic_material__user_material__category=FABRIC_TRIM_TYPES
        ).order_by('material__material_detail__generic_material__user_material__category')
        pack_placements = OrderPackPlacementMaterial.objects.filter(placement__order_pack__in=self.packs).exclude(
            material__material_detail__generic_material__user_material__category=FABRIC_TRIM_TYPES
        ).order_by('material__material_detail__generic_material__user_material__category')

        fabric_placements = OrderPackItemPlacementMaterial.objects.filter(
            placement__order_pack_item__pack__in=self.packs,
            material__material_detail__generic_material__user_material__category=FABRIC_TRIM_TYPES
        ).order_by('material__material_detail__generic_material__user_material__category')

        fabric_material_summary = self.get_fabric_material_summary(fabric_placements, 'orderpackitemplacementmaterialconsumption', get_quantity_func)
        sewing_trim_material_summary = self.get_material_summary(pack_item_placements, 'orderpackitemplacementmaterialconsumption', get_quantity_func)
        packing_trim_material_summary = self.get_material_summary(pack_placements, 'orderpackplacementmaterialconsumption', get_quantity_func_b)
        operation_costs = self.get_ie_operation_costs_summary(order_pack_items)
...
class OrderItemWiseSummary(MaterialCostingSummaryMixin):

    def get_ie_operation_costs(self, order_item):
        pack_items = OrderPackItem.objects.filter(pack__in=self.packs, item=order_item)
        return self.get_ie_operation_costs_summary(pack_items)

    def get_material_summary_for_pack_items(self):
        order_items = self.costing.get_order_items()
        item_summary = []
        get_quantity_func = lambda placement_material_object: placement_material_object.placement.order_pack_item.pack.cad_quantity
        ...
        for order_item in order_items:
            placement_materials = OrderPackItemPlacementMaterial.objects.filter(
                placement__order_pack_item__pack__in=self.packs,
                placement__order_pack_item__item=order_item).exclude(
                material__material_detail__generic_material__user_material__category=FABRIC_TRIM_TYPES
            )
            # TODO - major handle fabrics
            # TODO - major handle service costs
            operation_costs = self.get_ie_operation_costs(order_item)
            material_summary = self.get_material_summary(placement_materials, 'orderpackitemplacementmaterialconsumption', get_quantity_func)

            item_summary.append({
                'item_display': order_item.item_display,
                'item_id': order_item.pk,
                'sewing_trim_material_summary': material_summary,
