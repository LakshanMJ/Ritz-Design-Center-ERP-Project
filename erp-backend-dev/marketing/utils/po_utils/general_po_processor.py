...
class GeneralPOProcessor(POProcessorMixin):
    _po_text = ['Purchase Order', 'purchase order',]
    _style_text = ['Style', 'Style',]
    _colorway_text = ['Colorway', 'colorway']
    _size_text = ['Size', 'size']
    _quantity_text = ['Quantity', 'quantity']
    _country_text = ['Country', 'country']
    _delivery_date_text = ['Expected Shipment Date', 'expected shipment date']
    ...
    def get_po_data(self, sheet, po_column_idx, sytle_column_idx, colorway_column_idx, size_column_idx, quantity_column_idx, country_column_idx, delivery_date_column_idx):
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            country = row[country_column_idx]
            print(style_number, colorway)
