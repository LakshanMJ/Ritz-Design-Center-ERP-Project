...
class MandSPOProcessor(POProcessorMixin):
    ...
    def is_po_colorways_block(self, sheet, current_row_index, current_column_index):
        ...
        avail_cell_value = self.santize_value(sheet.cell(row=current_row_index, column=current_column_index + 2).value)
        channel_cell_value = self.santize_value(sheet.cell(row=current_row_index, column=current_column_index + 3).value)
        quantity_cell_value = self.santize_value(sheet.cell(row=current_row_index, column=current_column_index + 4).value)
        #print(fob_row_cell_value, avail_cell_value, channel_cell_value, quantity_cell_value)
        if fob_row_cell_value in self._fob_text and avail_cell_value in self._avail_text and channel_cell_value in self._channel_text and \
    ...
    def get_colorway(self, sheet, po_details_start_row_index):
        colorway = None
        colorway = sheet.cell(row=po_details_start_row_index, column=1).value
        return colorway
        ...
        for row_index in range(1, sheet.max_row + 1):
            for column_index in range(1, sheet.max_column + 1):
                if self.is_po_colorways_block(sheet, row_index, column_index):
                    ...
                    colorway = self.get_colorway(sheet, row_index)
                    ...
                    for row in sheet.iter_rows(min_row=row_index, max_row=last_row_in_colorway_range, min_col=sheet.min_column, max_col=last_column_in_colorway_range, values_only=True):
                        if row[fob_column_idx] is not None and row[channel_column_idx] is not None and row[avail_column_idx] is not None \
                            and row[quantity_column_idx] is not None:
                            if row[0] is None:
                                quantities = row[quantity_column_idx+1:last_column_in_colorway_range + 1]
                                #country = row[channel_column_idx] # TODO 2025/05/08 change this because of po have multiple countries and marketing costing have single country
                                country = None
