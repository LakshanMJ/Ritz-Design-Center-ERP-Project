...
import { allApprovalDetailPageURL, allTaskDetailPageURL, myApprovalDetailPageURL, myTaskDetailPageURL } from '@/helpers/constants/FrontEndUrls';
...
import { allPendingTaskListURL, myPendingTaskListURL, taskEntityURL } from '@/helpers/constants/RestUrls';
import RitzToggleButtonGroup from '@/components/Ritz/RitzToggleButtonGroup';
import CircularLoader from '@/components/CircularLoader';

const PendingTaskList = ({ taskType }: any) => {
  ...
  const loadEntityDetails = () => {
    const requests = [
      api.get(taskEntityURL(taskType == 'my_tasks' ? 'user' : 'admin', 'pending'))
    ]
    Promise.all(requests).then(response => {
  ...
  const columns: ColumnDef<any>[] = [
    {
      ...
      header: 'Task No',
      cell: props => (
        <Link target="_blank" component={NextLink} href={taskType == 'my_tasks' ? myTaskDetailPageURL(props.row.original.id) : allTaskDetailPageURL(props.row.original.id)}>{props.row.original.display_number}</Link>
      )
    },
    {
