...
const GeneralShadeGroupSummary = ({ sourceId, sourceDataUrl }: any) => {
  ...
  const handleImage = (materialId:any, shadeGroupId: any, shadeGroupeName: any, images: any) => {
    if (images[0]?.id != null) {
      ...
      api.post(GrnUrls.grnActualShadeGroupAttachmentUploadUrl(), dataToSend).then(resp => {
        console.log(resp)
      }).catch(error => {
        toast.error(getDefaultError(error?.response?.status));
      })
...
export default GeneralShadeGroupSummary;
