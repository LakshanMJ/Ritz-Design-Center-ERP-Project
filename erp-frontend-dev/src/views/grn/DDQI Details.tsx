...
const DDQIDetails = ({ supplierPoId, selectedDeliveryId }: any) => {
            ...
            {supplierPoDetails.map((deliveryData: any, deliveryIndex: any) => (
              <Grid item xs={12} sm={12} md={12} key={deliveryIndex}>
                <Card variant="outlined" key={deliveryIndex} sx={{ border: selectedDelivery?.id === deliveryData.id && '1px solid green', cursor: 'pointer' }} onClick={() => handleCardClick(deliveryData)}>
                  ...
                  />
                  <Table size="small" sx={{marginBottom: '1em', maxWidth: '600px'}}>
                    <TableHead>
                      <TableRow>
                  ...
                  <Table size="small">
                      <TableHead>
                        <TableRow>
                          ...
                          <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left', width: '200px' }}><Typography variant="body1" fontWeight="bold">QA Pass Quantity</Typography></TableCell>
                          <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left', width: '200px' }}><Typography variant="body1" fontWeight="bold">Rejected Quantity</Typography></TableCell>
                        </TableRow>
                      </TableHead>
                      <TableBody>
                        {/* Todo */}
                        {deliveryData?.materials?.length === 0 ? (
                          ...
                          deliveryData.materials?.map((materialData: any, materialIndex: any) => (
                            <TableRow key={materialIndex}>
                              ...
                              <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left' }}>{materialData?.grn_qa_passed_quantity?.quantity} {materialData?.grn_qa_passed_quantity?.quantity_units_display}</TableCell>
                              <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left' }}>{materialData?.grn_rejected_quantity?.quantity} {materialData?.grn_rejected_quantity?.quantity_units_display}</TableCell>
                            </TableRow>
                          ))
                        )}
