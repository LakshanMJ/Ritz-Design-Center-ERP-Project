...
import { outgoingCommercialInvoicesUrl, outgoingPaymentsListURL} from '@/helpers/constants/rest_urls/FinanceUrls';
import RitzTable from '@/components/Ritz/RitzTable';
import { ColumnDef } from '@tanstack/react-table';
import NextLink from 'next/link';
import { outgoingCommercialInvoiceDetailPageURL, outgoingPaymentDetailPageURL } from '@/helpers/constants/front_end/FinanceUrls';
...
const OutgoingCommercialInvoiceList = () => {
    ...
    const fetchData = ({
        ...
        const requests = [
            api.get(outgoingCommercialInvoicesUrl(pageIndexValue + 1, pageSizeValue, queryParams)),
        ];
    ...
    const columns: ColumnDef<any>[] = [
        {
            ...
            cell: props => (
                <Link component={NextLink} href={outgoingCommercialInvoiceDetailPageURL(props.row.original.id)}>{props.row.original.display_number}</Link>
            )
        },
        {
            accessorKey: 'amount',
    ...
    return (
        <>
            {isOpenCreateModal && (
                <RitzModal open={isOpenCreateModal} onClose={()=>{createModalOpen(false)}} title={"Create Outgoing Commercial Invoice"}>
                     <CreateOutgoingCommercialInvoice handleSavedData={handleSavedData} />
                </RitzModal>
            )}
            {isLoading ? (
                <DefaultLoader />
                ...
                <>
                    <Box>
                        <Typography variant='h1' color='text.primary' sx={{ mt: 2 }}>Outgoing Commercial Invoices</Typography>
                    </Box>
                    <Box>
                        <Button variant="contained" onClick={() => { createModalOpen(true) }} >Add Commercial Invoices</Button>
                        <RitzTable
...
export default OutgoingCommercialInvoiceList;
