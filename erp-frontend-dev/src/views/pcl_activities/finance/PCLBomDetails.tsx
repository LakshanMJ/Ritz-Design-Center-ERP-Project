...
import { Table, TableHead, TableRow, TableCell, TableBody, Box, Typography, useTheme, Tooltip, Link, TableContainer, Paper, Grid, IconButton } from '@mui/material';
...
const PCLBomDetails = ({ dataSet }: any) => {
                ...
                <>
                    <Box sx={{ mb: 1 }}>
                        <Grid container spacing={2}>
                            {colors.map((colorItem: any, index: any) => (
                        ...
                        <Table stickyHeader >
                            <TableHead>
                                <TableRow sx={{ background: theme.palette.grey[200], border: (theme) => `1px solid ${theme.palette.grey[100]}` }}>
                                    ...
                                    <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left' }}>Purchase Order</TableCell>
                                        ...
                                        {materialCategory.material_data?.map((material: any, materialIndex: any) => (
                                            material?.supplier_data?.map((spo: any, spoIndex: any) => (
                                                <React.Fragment key={`${keyHelper.getNextKeyValue()}`}>
                                                    <TableRow>
                                                        {spoIndex === 0 && (
                                                            <TableCell rowSpan={material?.supplier_data?.length}
                                                                sx={{
                                                                    border: (theme) => `1px solid ${theme.palette.grey[200]}`,
                                                                    width: '20%',
                                                                    textAlign: 'left',
                                                                ...
                                                                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                                                    <Typography sx={{ mr: 1 }} >{material?.attributes?.ritz_customer_brand_reference_code}</Typography>
                                                        ...
                                                        <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left' }}>
                                                            <Link sx={{ cursor: 'pointer' }} target={'_blank'} href={purchaseOrderDetailPageURL(spo?.purchase_order_id)}>{spo?.purchase_order}</Link>
                                                        </TableCell>
                                                        <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left' }}>{spo?.supplier_name}</TableCell>
                                                        <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left' }}>
                                                            <Link sx={{ cursor: 'pointer' }} target={'_blank'} href={spo?.attachment_file_path}>{spo?.supplier_po}</Link>
                                                        </TableCell>
                                                        <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left' }}>{spo?.required_quantity?.quantity} {spo?.required_quantity?.quantity_units_display}</TableCell>
                                                        <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left' }}>{spo?.order_quantity?.quantity} {spo?.order_quantity?.quantity_units_display}</TableCell>
                                                        <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left' }}>{formatAmount(spo?.price?.amount)} {spo?.price?.amount_currency_display}</TableCell>
                                                        <TableCell sx={{ border: (theme) => `1px solid ${theme.palette.grey[200]}`, textAlign: 'left' }}>{spo?.receiving_type}</TableCell>
                                                        {showMaterialGanttChart && (
