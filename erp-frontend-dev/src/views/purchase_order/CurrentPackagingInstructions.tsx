...
import PackagingVersion from '../packaging/PackagingVersion';
import { deletePurchaseOrderPackagingDetailsURL, purchaseOrderPackagingDetailsURL, savePurchaseOrderPackagingDetailsURL } from '@/helpers/constants/rest_urls/POUrls';
...
const PurchaseOrderPackagingInstruction = ({ dataList, packagingVersion, versionId, purchaseOrderId, packagingVersionType }: any) => {
  ...
  const fetchData = () => {
    let packagingData
    if (packagingVersionType == 'purchase_order') {
      packagingData = {
        po_packaging_id: packagingVersionId,
        pack_packaging_id: null
      }
    } else if(packagingVersionType == 'costing') {
      packagingData = {
        pack_packaging_id: packagingVersionId,
        po_packaging_id: null
      }
    }else if(packagingVersionType == null){
      packagingData = {
        po_packaging_id: null,
        pack_packaging_id: null
      }
    ...
    Promise.all([
      api.post(purchaseOrderPackagingDetailsURL(purchaseOrderId), packagingData),
      ...
      setPackagingVersionId(packagingData.po_packaging)
  ...
  const handleSave = () => {
    const request = {
      method: 'post',
      url: savePurchaseOrderPackagingDetailsURL(packagingVersionId),
  ...
  const handleDeletePackingInstruction = () => {
    if (selectedPackagingInstructionDeleteData.instructionId) {
      api.post(deletePurchaseOrderPackagingDetailsURL(selectedPackagingInstructionDeleteData.instructionId)).then(() => {
    ...
    <>
      {openVersionModal && (
        <RitzModal open={openVersionModal} title={"Select Packaging Version"} onClose={() => setOpenVersionModal(false)}>
         <ExcitingPackagingInstructions dataList={dataList} handleAcceptChange={handleAcceptChange} handleIgnoreChange={handleIgnoreChange} editVersion={true} />
        </RitzModal>
      )}
      {isDeleteModalOpen && (
        <RitzModal
        ...
        <React.Fragment>
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', mb: 1 }}>
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              <Button variant="contained" color="primary" disabled={!canEdit}  onClick={() => handleOpenVersionModal()}>P/V : {packagingDetails?.display_name}</Button>
            </Box>
          </Box>
