...
const PurchaseOrderFabricSummary = (purchaseOrderId: any) => {
  const [summary, setSummary] = useState([]);
  const [formattedData, setFormattedData] = useState([]);
  const [isLoading, setIsLoading] = useState(true);

    const fetchData = () => {
        api.get(grnUrls.purchaseOrderInhouseFabricSummaryUrl(purchaseOrderId?.purchaseOrderId))
            .then(resp => {
                ...
                responseData.map((row: any) => {
                    ...
                    let widthMappings = {};
                    row?.['width_groups']?.map((widthGroup: any) => {
        ...
        <Grid item xs={4} key={index}>
          <Card sx={{ mb: 3, height: '100%' }}>
            <Typography variant="h6" align="center" sx={{ mt: 3 }}>
              {summary[index]?.grn_material_details?.material_label} -{' '}
              {summary[index]?.grn_material_details?.ritz_customer_brand_reference_code} -{' '}
              {summary[index]?.grn_material_details?.reference_code}
            </Typography>
...
export default PurchaseOrderFabricSummary